...
- `apps/mobile` (`@mingle/mobile`) — ⭐ primary client. Expo SDK 56, Expo Router(file-based `app/`), expo-secure-store(JWT), expo-notifications+expo-device(push), expo-image-picker, expo-screen-orientation(매칭~파티 가로 lock), expo-haptics(네이티브 의미 기반 피드백), lucide-react-native + react-native-svg, reanimated 4, `@expo-google-fonts/gaegu`.
  ...
  - **폰트**: Gaegu + 로컬 Pretendard Regular/SemiBold를 `app/_layout.tsx` `useFonts`로 함께 로드(로드 전 브랜드 로딩 상태). `fonts.display`는 워드마크·타이틀·버튼 라벨용, 긴 한글 본문·라벨·메타데이터는 Pretendard 토큰 사용(Gaegu 14px 미만 금지).
...
> **의도된 예외(되돌리지 말 것)**: 파티 **게임 월드(맵+캐릭터)**는 두들 라인아트가 아니라 **Kenney 로그라이크(CC0) 픽셀 아트**다(2026-07-21 사용자 지시 — 손그림 두들 품질 불만족으로 기성 픽셀 에셋 채택, 맵↔캐릭터 결 통일). 게임 월드 밖(홈·채팅·설정·프로포즈 등 앱 UI + 파티 오버레이 카드/채팅/멤버시트/어몽 회의·밸런스 모달)은 두들 유지. 게임 월드를 "두들로 통일" 하지 말 것.
...
- **팔레트**: ink `#17150F` / paper `#FFFFFF` / grays `#786F67`·`#D9D5CC` / fills `#F1EFE9`·`#E7E4DC` / **grayDark = 웜 토프 `#736357`**. 포인트: **`accent #D13F4F`**(white text AA 통과 primary CTA·활성 탭·안읽음 배지), `accentBright #FF5864`(장식용 브랜드 코랄), `accentDeep #A92F3D`(눌림), `accentSoft #FF8276`, `accentFill #FF9F9D`, `onAccent #FFF`. 안전 의미는 브랜드 코랄과 분리: `danger #7B2531`+`dangerFill #F7E7EA`, `warning #9A5D00`+`warningFill #FFF0D6`. **화면당 primary 1개** — 나머지는 잉크-온-화이트. 대형 강조는 잉크 반전 블록(홈 히어로).
- **Hierarchy**: primary = 코랄 `DoodleButton variant="primary"`(잉크 보더 유지); secondary = paper+잉크 외곽. 위험 플로우 진입은 `danger`(paper+버건디 외곽), 되돌릴 수 없는 최종 확인만 `dangerSolid`(버건디 채움)이며 안전·동의 문구는 `serious` Pretendard 라벨을 사용. 아이콘 = Lucide outline. **UI에 이모지 금지**(2026-07-16 규칙 — 컬러 이모지는 SVG 아이콘으로; ✓·★ 등 모노크롬 기호는 허용). `DoodleButton`은 FLAT(그림자 없음) — 하드 오프셋 잉크 그림자는 카드류만.
- **RN gotchas**:
