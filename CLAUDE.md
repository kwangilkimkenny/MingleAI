...
## Data model (v2, `apps/backend/prisma/schema.prisma` — 21 models)
...
- **현재 상태**: Phases 0–6d + 두들 디자인 이식 + 코랄 팔레트 + 파티=게임 월드 재구성 + "AI를 찾아라" 변형 + 출시 준비(rate limit·연령 게이트·CORS) 전부 origin/megahuni에 푸시. 2026-07-22 **블라인드 스피드 데이트(v1 슬라이스)** 추가 — 성별 매칭·3단계 리빌 로테이션·10초 상호선택·LiveKit 토큰(미디어 seam은 avatar-only, 실연동은 EAS 단계). 최신 자동화: backend **380/380**, client-core **78/78**, shared **27/27**, mobile **123/123**(총 **608/608**), backend·mobile tsc·루트 lint(경고 0)·루트 production build green. **미적용/미검증**(사용자 실행 필요): 스피드데이트 DB 마이그레이션(`20260722090000_speed_date`), `livekit-server-sdk`/`@livekit/*` 설치, 웹/iOS/Android Expo export 재확인, mega-qa 재실행, EAS dev build 실기기. 웹·백엔드·레거시 MCP는 ESLint 9 flat config이며 Next/SWC는 15.5.21.
