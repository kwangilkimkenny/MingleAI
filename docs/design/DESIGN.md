...
이 문서는 두들 렌더링의 **방식과 원리**(§3)와 위계 원칙의 소스다.
> ⚠️ **색 체계(§1)는 2026-07-15 사용자 지시로 대체됨** — 순수 흑백이 아니라 **잉크 라인아트 + 흰 배경 +
> 코랄 포인트 팔레트**(`accent #FF5864` / `accentSoft #FF8276` / `accentFill #FF9F9D` / `accentDeep #E5424E`(파생),
> `grayDark`는 웜 토프 `#736357`)가 현행이다. **라이브 소스 오브 트루스 = `apps/mobile/src/lib/theme.ts` +
> 루트 CLAUDE.md 디자인 섹션.** §1의 "유채색 0" 규칙과 "강조=반전 블록" 규칙은 역사적 기준으로만 남긴다
> (반전 잉크 블록은 여전히 대형 강조에 사용, 포인트는 코랄).
